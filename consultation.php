...
<?php
      ...
      if (!empty($search)) {
          $sql = "SELECT COUNT(*) AS total FROM consultations WHERE 
                  Id LIKE :search OR 
                  Date_de_consultation LIKE :search OR
                  Heure_de_consultation LIKE :search OR
                  Id_patient  LIKE :search OR
                  Id_médecin LIKE :search OR
                  Motif_de_consultation LIKE :search
                  Diagnostic LIKe :search OR
                  TRaitement LIKE :search ";
      ...
      $offset = ($page - 1) * $limit;
      if (!empty($search)) {
          $sql = "SELECT * FROM consultations WHERE 
                  Id LIKE :search OR 
                  Date_de_rendez_vous LIKE :search OR 
                  Heure_de_rendez_vous LIKE :search OR
                  Id_patient LIKE :search OR
                  Id_médecin LIKE :search OR
                  Motif_de_consultation LIKE :search OR
                  Action  LIKE :search OR 
                  LIMIT :limit OFFSET :offset";
      } else {
          $sql = "SELECT * FROM consultations LIMIT :limit OFFSET :offset";
      }
      ...
      ?>
          ...
          <div class="row mb-1 bg-light">
            <div class="col-sm-3 col-lg-3">
              <a href="ajouter_consultation.php" class="btn btn-outline-primary me-2">
                <i class="bi bi-plus bi-2x"></i>
                Ajouter un consultations
              </a>
            </div>
          ...
          <table class="table table-striped table-bordered">
            ...
            <?php
              ...
              while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                echo "<tr>"; 
                echo "<td>".$row["Id"]."</td>";
                echo "<td>".$row["Date_de_consultation"]."</td>";
                echo "<td>".$row["Heure_de_consultation"]."</td>";
                echo "<td>".$row["Id_patient"]."</td>";
                echo "<td>".$row["Id_médecin"]."</td>"; 
                echo "<td>".$row["Motif_de_consultation"]."</td>"; 
                echo "<td>".$row["Diagnostic"]."</td>"; 
                echo "<td>".$row["Traitement"]."</td>"; 
                echo "<td>"; 
                echo "<a href='edit_consultation.php?id=".$row['Id']."'class='btn btn-outline-primary btn-sm'><i class='bi bi-pencil'></i></a>";
                echo "<a href='delete_consultation.php?id=".$row['Id']."'class='btn btn-outline-danger btn-sm'><i class='bi bi-trash'></i></a>";
                echo "<a href='afficher_consultation.php?id=".$row['Id']."'class='btn btn-outline-success btn-sm'><i class='bi bi-eye'></i></a>";
                echo "</td>";
                echo "</tr>";
              }
            ?>
