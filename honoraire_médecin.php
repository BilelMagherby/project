...
<div class="container">
  <div class="row">
    <div class="col-lg-12 d-flex flex-column justify-content-center">
      <h1 data-aos="fade-up">Liste des  honoraire_medecins</h1>
...
<?php
      ...
      if (!empty($search)) {
          $sql = "SELECT COUNT(*) AS total FROM honoraire_médecin WHERE 
                  Id LIKE :search OR 
                  Id_médecin  LIKE :search OR
                  Année LIKE :search OR
                  Mois LIKE :search OR
                  Montant_Versé LIKE :search OR
                  Adresse_email LIKE :search";
          $stmt = $pdo->prepare($sql);
          $stmt->bindValue(':search', '%' . $search . '%', PDO::PARAM_STR);
          $stmt->execute();
      } else {
          $sql = "SELECT COUNT(*) AS total FROM honoraire_médecin";
          $stmt = $pdo->query($sql);
      }
      ...
      if (!empty($search)) {
          ...
          $sql = "SELECT * FROM honoraire_médecin LIMIT :limit OFFSET :offset";
      }
      ...
      ?>
          ...
          <div class="row mb-1 bg-light">
            <div class="col-sm-3 col-lg-3">
              <a href="ajout_médecins.php" class="btn btn-outline-primary me-2">
                <i class="bi bi-plus bi-2x"></i>
                Ajouter un honoraire_médecin
              </a>
            </div>
          ...
          <table class="table table-striped table-bordered">
            ...
            <?php
              ...
              while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo "<td>".$row["Mois"]."</td>"; 
                echo "<td>".$row["Montant_versé"]."</td>";
                echo "<td>"; 
                echo "<a href='edit-honoraire_medecins.php?id=".$row['Id']."'class='btn btn-outline-primary btn-sm'><i class='bi bi-pencil'></i></a>";
                echo "<a href='delete_honoraire_médecin.php?id=".$row['Id']."'class='btn btn-outline-danger btn-sm'><i class='bi bi-trash'></i></a>";
